...
pub async fn leave_channel(
    ...
    manager
        ...
        .remove(GuildId(dwc.guild_id.parse().context(
            ChannelControlError::GuildIDParsingFailed.to_string(),
        )?))
        .await
...
impl VoiceEventHandler for TrackEndNotifier {
    async fn act(&self, _ctx: &EventContext<'_>) -> Option<Event> {
        ...
        match x {
            ...
            Err(_e) => {
                error!(
                    "Failed to notify job: {} that track has ended.",
...
pub async fn join_channel(
    ...
    let gid = guild_id
        .parse()
        .context(ChannelControlError::GuildIDParsingFailed.to_string())?;
    let vcid = voice_channel_id
        .parse()
        .context(ChannelControlError::ChannelIDParsingFailed.to_string())?;
